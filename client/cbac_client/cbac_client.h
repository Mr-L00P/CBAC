...
#define CBAC_RESERV_DELETED 5  // Reservation has been deleted
#define CBAC_WRONG_USER     6  // No reservation, occupied space                         Message empty
#define CBAC_EMPTY_SPACE    7  // No reservation but empty space                         Message empty
#define CBAC_API_ERROR      8  // Daemon couldn't process request with Google API        Message set to origin of the error
#define CBAC_PARAM_ERROR    9  // Params given to daemon not valid                       Message set to origin of the error
#define CBAC_OCCUPIED       10 // Time supplied overlaps with event in the calendar      Message informative

// Packet request codes
#define CBAC_CHECK_RESERV   11 // Asks daemon to check if user can go through.               Message set to username to check
#define CBAC_MAKE_RESERV    12 // Asks daemon to make a reservation from now                 Message set to user, when, time interval, separated by spaces
#define CBAC_DEL_RESERV     13 // Asks daemon to delete a certain event                      Message set to timestamp intersecting with the event
#define CBAC_ADD_USER       14 // Asks daemon to add user to the calendar of the system.     Message set to user's email address and role, separated by space
#define CBAC_DEL_USER       15 // Asks daemon to delete user from the calendar               Message set to user's email address
#define CBAC_UPDATE_CONF    16 // Asks daemon to update env variables                        Message empty
#define CBAC_EXTEND_RESERV  17 // Asks daemon to extend the current reservation              Message set to user and minutes to extend
